...
class _DecoderLayer(nn.Module):
    ...
    def forward(self, queries: torch.Tensor,
        ...
        Args:
            ...
            cross_attn_bias: optional ``(num_queries, num_spatial)`` additive
                bias added to cross-attention logits before softmax.
...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        self.feat_h = feat_h
        self.feat_w = feat_w
        self.joint_row_prior = joint_row_prior
        ...
        # Learnable cross-attention spatial bias
    ...
    def _init_head_weights(self) -> None:
        ...
        # Warm-start row biases with Gaussians for body joints (design003 only)
        if (self.use_cross_attn_bias
                and self.cross_attn_bias_type == 'factored_warmstart'
                and self.joint_row_prior is not None):
            h_coords = torch.arange(self.feat_h, dtype=torch.float32)  # (feat_h,)
            sigma = 4.0
            alpha = 1.0
            ...
            # hand joints (indices 22–69) remain zero-initialized
    ...
    def forward(
        ...
        if self.use_cross_attn_bias:
            if self.cross_attn_bias_type == 'full':
                bias = self.cross_attn_bias     # (num_joints, feat_h * feat_w)
            else:
                # Factored: outer sum via broadcasting
                # cross_attn_bias_row: (num_joints, feat_h) → unsqueeze(-1) → (num_joints, feat_h, 1)
                # cross_attn_bias_col: (num_joints, feat_w) → unsqueeze(-2) → (num_joints, 1, feat_w)
                # sum → (num_joints, feat_h, feat_w), then flatten → (num_joints, feat_h * feat_w)
                bias = (self.cross_attn_bias_row.unsqueeze(-1) +
                        self.cross_attn_bias_col.unsqueeze(-2))
                bias = bias.view(self.num_joints, -1)
