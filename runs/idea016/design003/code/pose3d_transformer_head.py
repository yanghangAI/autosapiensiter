...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        self._film_block_h = None
        self._film_block_w = None
    ...
    def forward(
        ...
        if self.film_pool_type == 'spatial_block':
            ...
            # Average pool within each block over dims 2 (rows) and 4 (cols):
            ...
            gamma_expanded = gamma_b.unsqueeze(2).unsqueeze(4).expand(B, 4, H // 4, 4, W // 4, D)
            beta_expanded  = beta_b.unsqueeze(2).unsqueeze(4).expand(B, 4, H // 4, 4, W // 4, D)
