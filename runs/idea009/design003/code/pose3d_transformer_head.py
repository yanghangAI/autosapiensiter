...
class _DecoderLayer(nn.Module):
    ...
    def forward(self, queries: torch.Tensor,
        ...
        Args:
            ...
            spatial_drop_prob: Fraction of spatial tokens to mask during
                training cross-attention. Ignored at inference.
        ...
        if self.training and spatial_drop_prob > 0.0:
            ...
            # Shape: (B, N_spatial). Generated fresh every forward call.
...
class Pose3dTransformerHead(BaseHead):
    ...
    Args:
        ...
        spatial_drop_prob_start (float): Initial spatial token dropout probability
            (at epoch 1). Updated per epoch by SpatialDropAnnealHook.
        spatial_drop_prob_end (float): Final spatial token dropout probability
            (at epoch num_epochs).
    ...
    def forward(
        ...
        # Decoder
        decoded = self.decoder_layer(queries, spatial, spatial_drop_prob=self.spatial_drop_prob)  # (B, num_joints, hidden_dim)
