...
Architecture::
    ...
    feats[-1]  (B, C, H', W')
        ...
        → transformer decoder (1 layer):
            ...
            cross-attention: queries attend to spatial tokens (with learnable-sigma depth gate)
            FFN with residual
...
class Pose3dTransformerHead(BaseHead):
    ...
    Args:
        ...
        depth_gate_type (str): Type of depth gate ('none' or 'gaussian_learnable_sigma').
        depth_probe_loss_weight (float): Weight for auxiliary probe loss (0.0 disables).
    ...
    def __init__(
        ...
        if depth_gate_type == 'gaussian_learnable_sigma':
            ...
            # Learnable log-sigma: init to 0.0 → sigma = exp(0) = 1.0
    ...
    def _init_head_weights(self) -> None:
        ...
        if self.depth_gate_type == 'gaussian_learnable_sigma':
            ...
            # log_sigma is already initialized to 0.0 via torch.zeros(1)
    ...
    def forward(
        ...
        if self.depth_gate_type == 'gaussian_learnable_sigma':
            ...
            # Learnable sigma, clamped to >= 0.01 to prevent division by zero
            sigma = torch.exp(self.log_sigma).clamp(min=0.01)     # scalar
            depth_err = (z_tok - z_hat) / sigma                    # (B, H*W)
            attn_logit_bias = -0.5 * depth_err ** 2               # (B, H*W), <= 0
            # Cache z_hat for auxiliary loss in loss()
