...
Architecture (Design 003: 22-query body-only + depth gate)::

    feats[-1]  (B, C, H', W')
        → flatten to (B, H'*W', C)
        → input_proj: Linear(C, hidden_dim)
        → add 2D sinusoidal positional encoding
        → depth gate: two linear probes compute per-token Gaussian log-gate
        → transformer decoder (1 layer, 22 body queries):
            self-attention over 22 body joint queries
            cross-attention: queries attend to spatial tokens (with depth gate)
            FFN with residual
        → Linear(hidden_dim, 3) per body token  → body_joints  (B, 22, 3)
        → Linear(22*hidden_dim, 48*3)            → hand_joints  (B, 48, 3)
        → cat([body_joints, hand_joints])         → joints       (B, 70, 3)
...
class _DecoderLayer(nn.Module):
    ...
    def forward(self, queries: torch.Tensor,
        ...
        Args:
            ...
            attn_logit_bias: ``(B, num_spatial)`` additive bias to cross-attn
                             logits, or None for standard cross-attention.
...
class Pose3dTransformerHead(BaseHead):
    ...
    Combines 22-query body-only decoder with fixed-sigma Gaussian depth gate.

    Args:
        in_channels (int): Embedding dimension from the backbone (e.g. 1024).
        hidden_dim (int): Internal dimension for the decoder.
        num_joints (int): Total number of output joints (70 for BEDLAM2).
        ...
        num_body_queries (int): Number of body-only decoder queries (22 for
            body-only mode; when equal to num_joints, identical to baseline).
        hand_aux_loss_weight (float): Weight for auxiliary hand joint loss.
        depth_gate_type (str): Type of depth gate ('none' or 'gaussian').
        depth_gate_sigma (float): Fixed sigma for Gaussian depth gate.
    ...
    def __init__(
        ...
        # Learnable joint query embeddings — use num_body_queries
        ...
        # Hand projection (only when num_body_queries < num_joints)
    ...
    def _init_head_weights(self) -> None:
        ...
        # Depth probe zero-init: gate is flat at step 0 → identical to baseline
    ...
    def forward(
        ...
        # Broadcast body joint queries to batch
        queries = self.joint_queries.weight.unsqueeze(0).expand(
            B, -1, -1)  # (B, num_body_queries, hidden_dim)

        # Compute depth gate logits (or None for baseline behavior)
        ...
        pelvis_token = decoded[:, 0, :]          # (B, hidden_dim) — token 0
    ...
    def loss(
        ...
        # Auxiliary hand loss — anchors hand_proj in pose space
