...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        # Transformer decoder (N layers, independent weights)
    ...
    def forward(
        ...
        # Decoder (N layers), collect all layer outputs
        decoded = queries
        layer_outputs = []
        ...
        # Output projections (shared joints_out across all layers; pelvis from final only)
        final = layer_outputs[-1]  # (B, num_joints, hidden_dim)
        joints = self.joints_out(final)  # (B, num_joints, 3)

        pelvis_token = final[:, 0, :]  # (B, hidden_dim)
