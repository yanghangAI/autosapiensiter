...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        # Learnable joint query embeddings (num_body_queries instead of num_joints)
    ...
    def forward(
        ...
        # Broadcast joint queries to batch (num_body_queries, not num_joints)
        queries = self.joint_queries.weight.unsqueeze(0).expand(
            B, -1, -1)  # (B, num_body_queries, hidden_dim)

        # Decoder operates on body queries only
        decoded = self.decoder_layer(queries, spatial)  # (B, num_body_queries, hidden_dim)

        # Output projections — body joints only
        body_joints = self.joints_out(decoded)  # (B, num_body_queries, 3)

        # Zero-pad hand joints (indices num_body_queries to num_joints-1)
        pad = torch.zeros(B, self.num_joints - self.num_body_queries, 3,
                          device=body_joints.device, dtype=body_joints.dtype)
        joints = torch.cat([body_joints, pad], dim=1)  # (B, num_joints, 3)
