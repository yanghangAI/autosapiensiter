...
from pelvis_utils import uv_to_grid_coords, build_gaussian_heatmap_2d
...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        self.use_uv_heatmap = bool(use_uv_heatmap)
    ...
    def _init_head_weights(self) -> None:
        ...
        # Output projections
        _out_modules = [self.joints_out, self.depth_out]
        if self.uv_out is not None:
            _out_modules.append(self.uv_out)
    ...
    def forward(
        ...
        if self.use_uv_heatmap:
            H_, W_ = self.feat_h, self.feat_w
            assert spatial.shape[1] == H_ * W_, (
                f"spatial token count {spatial.shape[1]} != feat_h*feat_w={H_*W_}; "
            ...
            attn_hw = uv_attn.view(-1, H_, W_)
            h_idx = torch.arange(H_, device=spatial.device, dtype=attn_hw.dtype)
            w_idx = torch.arange(W_, device=spatial.device, dtype=attn_hw.dtype)
            v_frac = (attn_hw.sum(dim=-1) * h_idx).sum(dim=-1) / max(H_ - 1, 1)
            u_frac = (attn_hw.sum(dim=-2) * w_idx).sum(dim=-1) / max(W_ - 1, 1)
            pelvis_uv = torch.stack(
                [u_frac * 2.0 - 1.0, v_frac * 2.0 - 1.0], dim=-1)
            self._uv_attn = uv_attn
        else:
    ...
    def loss(
        ...
        if (self.use_uv_heatmap
                and self.uv_heatmap_loss_weight > 0.0
                and self._uv_attn is not None):
            gt_grid = uv_to_grid_coords(gt_uv, self.feat_h, self.feat_w).detach()
