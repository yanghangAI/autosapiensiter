...
When ``decouple_pelvis=True``, an additional ``pelvis_query`` (1 × hidden_dim)
is introduced. It runs through the same ``decoder_layer`` weights via a separate
cross-attention-only call (no self-attention mixing with joint queries). The
pelvis depth/UV heads read from this dedicated token instead of from
``decoded[:, 0, :]``, cleanly separating body-structure and absolute-localisation
tasks.
"""
...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        # Optional dedicated pelvis query (decoupled pathway)
