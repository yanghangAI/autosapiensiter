...
When ``decouple_pelvis=True`` and ``pelvis_decoder_type='independent'``, an
additional ``pelvis_query`` (1 × hidden_dim) and a fully independent
``pelvis_decoder`` (_DecoderLayer with separate weights) are introduced. The
pelvis query runs through ``pelvis_decoder`` via cross-attention only (self-
attention skipped for single-token efficiency). Pelvis depth/UV heads read from
this dedicated token instead of from ``decoded[:, 0, :]``.
"""
...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        # Optional dedicated pelvis query and independent decoder
    ...
    def _init_head_weights(self) -> None:
        ...
        # Pelvis query embedding (if decoupled); pelvis_decoder Linear layers
        # are initialised by PyTorch default (xavier uniform) — no extra init needed.
