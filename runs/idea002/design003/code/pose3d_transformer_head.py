...
When ``decouple_pelvis=True`` and ``pelvis_decoder_type='depth_fused'``, an
additional ``pelvis_query`` (1 × hidden_dim), an independent ``pelvis_decoder``
(_DecoderLayer), and a ``depth_proj`` (Linear(hidden_dim, hidden_dim)) are
introduced. A global depth-context token is computed by mean-pooling ``spatial``
over the spatial dimension and projecting through ``depth_proj``. This token is
prepended to ``spatial`` to form ``spatial_with_depth`` (B, H*W+1, hidden_dim)
before pelvis cross-attention. Joint queries use the original ``spatial``
unchanged. The pelvis decoder runs cross-attention only (self-attention skipped).
"""
...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        # Optional dedicated pelvis query, independent decoder, and depth projection
