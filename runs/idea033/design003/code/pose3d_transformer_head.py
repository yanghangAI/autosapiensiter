...
class Pose3dTransformerHead(BaseHead):
    ...
    Args:
        ...
        use_k_film (bool): Whether to use camera-intrinsic FiLM conditioning.
        k_film_variant (str): Which FiLM variant: 'query', 'spatial', or 'pelvis'.
        k_film_hidden (int): Hidden width for the FiLM MLP.
    ...
    def __init__(
        ...
        # ── Camera-intrinsic FiLM conditioning ──
    ...
    def _build_k_batch(self, batch_data_samples, device) -> torch.Tensor:
        """Build (B, 6) normalized K tensor from per-sample metainfo."""
        vals = []
    ...
    def forward(
        ...
        k_batch: Optional[torch.Tensor] = None,
        ...
        Args:
            feats: Tuple of feature tensors from the backbone.
                   ``feats[-1]`` is ``(B, C, H', W')``.
            k_batch: Optional (B, 6) normalized K tensor for FiLM conditioning.
        ...
        # ── Variant B: FiLM on spatial tokens ──
        ...
        # ── Variant C: FiLM on pelvis token only ──
