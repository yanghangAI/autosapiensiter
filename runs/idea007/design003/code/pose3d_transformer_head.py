...
class _DecoderLayer(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int = 8, dropout: float = 0.1,
        ...
        # Store routing flags for use in forward
        self._per_head = per_head_routing
        self._num_heads = num_heads

        if per_head_routing:
            # Per-head learnable bias: (num_heads, num_joints, num_spatial), zero-init
            self.cross_attn_bias = nn.Parameter(
                torch.zeros(num_heads, num_joints, num_spatial))
        else:
            # ── Shared cross-attention spatial routing bias ────────────────────
            ...
            if cross_attn_bias_init == 'band_prior':
                ...
                sigma = 5.0
                g_lower = torch.exp(-0.5 * ((row_idx - 30.0) / sigma) ** 2)
                g_upper = torch.exp(-0.5 * ((row_idx - 10.0) / sigma) ** 2)
                bias_lower = g_lower - 0.5
                bias_upper = g_upper - 0.5
    ...
    def forward(self, queries: torch.Tensor,
        ...
        Args:
            ...
            B: batch size; required when ``per_head_routing=True`` to expand the
               per-head bias to ``(B * num_heads, num_joints, num_spatial)``.
        ...
        if self._per_head:
            ...
            # NOTE: B must be passed explicitly by Pose3dTransformerHead.forward
            bias_expanded = (
...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        # Map cross_routing_type to decoder layer kwargs
    ...
    def forward(
        ...
        # Decoder — pass B so per-head routing can expand bias to (B*num_heads, J, S)
