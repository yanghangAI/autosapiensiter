...
class Pose3dTransformerHead(BaseHead):
    ...
    def __init__(
        ...
        if dist_loss_mode == 'bone_weighted':
            ...
            # Build a (22, 22) is_bone mask, then gather upper-tri entries in
            # the same order as torch.triu_indices(22, 22, offset=1).
            ...
            for child, parent in enumerate(bone_parents):
                ...
                j = max(child, parent)
                is_bone[i, j] = True
                is_bone[j, i] = True  # symmetric for robustness
