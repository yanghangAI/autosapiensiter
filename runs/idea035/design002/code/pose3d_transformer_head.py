...
class DepthAblationDataPreprocessor(RGBDPoseDataPreprocessor):
    ...
    mode:
        'zero'    -- fill depth channel with 0.0.
        'gauss'   -- replace depth channel with torch.randn_like(depth).
        'shuffle' -- per-sample random pixel permutation of the depth
                     channel (preserves marginal histogram, destroys
                     spatial alignment).
