...
Architecture (iterative refinement variant, shared decoder weights, no
intermediate supervision)::

    feats[-1]  (B, C, H', W')
        → flatten + input_proj + 2D sincos pos enc → spatial tokens
        → Pass 1: decoder_layer(queries, spatial) → decoded_1
            joints_1 = joints_out(decoded_1)
        → Pass 2: queries_2 = decoded_1 + coord_enc(joints_1)   # zero-init
            decoded_2 = decoder_layer(queries_2, spatial)       # shared
            joints_residual = joints_out(decoded_2)
            joints_final = joints_1 + joints_residual
        → pelvis_depth, pelvis_uv read from decoded_2[:, 0, :]
"""
...
class Pose3dTransformerHead(BaseHead):
    ...
    Args:
        ...
        num_refine_passes (int): Number of decoder passes. ``1`` reproduces
            baseline behaviour (single pass, pelvis from pass-1 token 0).
            ``2`` runs the two-pass coordinate-conditioned refinement.
        shared_decoder (bool): If True, pass 2 reuses ``self.decoder_layer``.
            If False, a separate ``self.decoder_layer_2`` is constructed and
            used for pass 2. Only meaningful when ``num_refine_passes >= 2``.
        intermediate_supervision_weight (float): Weight of the auxiliary loss
            applied to the pass-1 joint output ``joints_initial``. ``0.0``
            disables it (Design 001 default).
    ...
    def __init__(
        ...
        # Coordinate encoder: maps pass-1 joint coordinates back into hidden_dim
        # so they can be added to the pass-2 queries. The final Linear is
        # zero-initialized (see ``_init_head_weights``), which makes this
        # module a no-op at initialization and preserves baseline-equivalent
        ...
        # Optional independent pass-2 decoder layer. Only constructed when
        # ``shared_decoder=False`` AND at least one refinement pass is enabled
        # (``num_refine_passes >= 2``). When ``shared_decoder=True`` this
        # attribute MUST NOT exist, so that the optimizer does not see any
        # unused parameters (preserves Design 001/002 behaviour exactly).
        if (not self.shared_decoder) and self.num_refine_passes >= 2:
            self.decoder_layer_2 = _DecoderLayer(
                hidden_dim, num_heads, dropout)

        # Output projections (shared across passes — standard Deformable-DETR
    ...
    def _init_head_weights(self) -> None:
        ...
        # Coordinate encoder: trunc_normal on the first Linear, zero-init on
        # the final Linear (weight AND bias) so ``coord_enc(joints_1) == 0``
        # at initialization and the refined queries match ``decoded_1``.
    ...
    def forward(
        ...
        Returns:
            Dict with keys:
                ``joints``:         ``(B, num_joints, 3)`` refined (final)
                                    root-relative joint coordinates in metres.
                ``joints_initial``: ``(B, num_joints, 3)`` pass-1 joint
                                    coordinates. Used by ``loss()`` when
                                    ``intermediate_supervision_weight > 0``.
                ``pelvis_depth``:   ``(B, 1)`` pelvis forward distance metres.
                ``pelvis_uv``:      ``(B, 2)`` pelvis (u, v) normalised.
        """
    ...
    def loss(
        ...
        # Optional intermediate supervision on the pass-1 body joints.
        # Design 001: intermediate_supervision_weight=0.0, so this branch is
        # skipped and the losses dict is identical in shape to the baseline.
