...
auxiliary intermediate supervision, and learnable per-joint sigma/gamma)::
    ...
    feats[-1]  (B, C, H', W')
        ...
        → compute dynamic Gaussian cross-attention bias from layer-0 predictions
          using learnable per-joint sigma (F.softplus) and gamma
...
class Pose3dTransformerHead(BaseHead):
    ...
    Design C: 2-layer decoder with learnable per-joint Gaussian bias AND auxiliary
    body-joint loss (weight=0.4) on layer-0 output. Learnable sigma (passed through
    F.softplus for positivity) and gamma allow per-joint attention focus widths:
    narrow for distal joints (wrists, ankles), broad for proximal joints.
    ...
    Args:
        ...
        reproj_bias_sigma (float): Initial Gaussian bandwidth (used as init for learnable).
        reproj_bias_gamma (float): Initial Gaussian amplitude (used as init for learnable).
    ...
    def loss(
        ...
        if self.use_reproj_bias and self.num_decoder_layers > 1:
            ...
            # Design C: run layer 0 with normal autograd so the auxiliary loss
            # can backpropagate through layer-0 weights
            ...
            # Design C: learnable per-joint sigma and gamma
